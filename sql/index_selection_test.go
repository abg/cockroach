...
func TestMakeConstraints(t *testing.T) {
	...
	testData := []struct {
		...
		*/
		...
		{`ST_DISTANCE(k, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < .1`, `k`, `[k >= 5/20, k <= 5/21]`},
	}
...
func TestMakeSpans(t *testing.T) {
	...
	testData := []struct {
		...
		{`ST_DISTANCE(k, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < .0000001`, `k`, `/-5693121811630260224-/-5693121811613483008`, `/-5693121811621871616-/-5693121811638648832`},
		// Verify a query without an index on k does a full scan.
		{`ST_DISTANCE(k, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < 2`, `a`, `-`, `-`},
	}
	for _, d := range testData {
